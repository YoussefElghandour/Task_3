...
export default function AllPerks() {
  ...
  // skip first run for the auto-search effect to avoid double fetch
  const didMountRef = useRef(false)

  // Derive unique merchants whenever perks change
  useEffect(() => {
  ...
  async function loadAllPerks() {
    ...
    try {
      ...
      setPerks([]) // ensure UI clears on error
  ...
  // useEffect #1: Initial Data Loading
  useEffect(() => {
    loadAllPerks()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  // useEffect #2: Auto-search on input change (debounced)
  useEffect(() => {
  ...
  function handleSearch(e) {
    e.preventDefault()
    loadAllPerks() // immediate fetch ignoring debounce
  }

  function handleReset() {
    setSearchQuery('')
    setMerchantFilter('')
    // auto-effect will refetch after state updates (debounced)
  }
      ...
      <div className="card">
        <form onSubmit={handleSearch} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Merchant Filter Dropdown - Controlled */}
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-2">
          ...
          <div className="flex gap-3 items-center">
            ...
            <button type="button" onClick={handleReset} className="btn" disabled={loading}>
